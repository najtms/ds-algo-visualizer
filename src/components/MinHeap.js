...
function MinHeap() {
  ...
  const [message, setMessage] = useState('Enter values to build the Min Heap');
    ...
    while (true) {
      ...
      if (right < heapSize) {
        steps.push({
          ...
          array: [...arr],
          message: `Comparing ${arr[smallest]} with right child ${arr[right]}`
        });
      ...
      if (smallest !== current) {
        steps.push({
          ...
          array: [...arr],
          message: `Swapping ${arr[current]} with ${arr[smallest]}`
        });
  ...
  const handleDelete = () => {
    ...
    newSteps.push({
      ...
      value: minValue,
      array: [...newHeap],
      message: `Extracting minimum: ${minValue}`
    });
    ...
    newSteps.push({
      action: 'complete',
      array: [...newHeap],
      message: `Deleted minimum ${minValue}`
    });
  ...
  useEffect(() => {
    ...
    } else if (step.action === 'extractMin') {
  ...
  return (
    <div className="min-heap-container">
      <div className="algorithm-info">
        <h2>Min Heap</h2>
        <p>
          A Min Heap is a complete binary tree where each parent node is less than or equal to its children.
          The minimum element is always at the root. Supports efficient insertion and extraction of minimum element.
        </p>
      </div>
      ...
      <div className="tree-visualization">
        ...
        {heap.length === 0 && (
          <div className="empty-tree-message">
            Heap is empty. Insert values to build the Min Heap.
          </div>
        )}
      </div>
